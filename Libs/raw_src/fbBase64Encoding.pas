...
VAR INPUT
  ...
  OutputBuffer: DWORD; (* Address (ADR) of the destination data converted in Base 64 *)
END VAR
...
{flag nowatch on}
VAR
  (* Steps of the general FOR *)
  nSteps: DWORD;
  (* Steps of the Base 64 Index *)
  nIndex: UINT;
  (* Steps to send the Base 64 converted *)
  nSend: UINT;
  (* Pointer to the input array *)
  pByteArray: POINTER TO BYTE;
  (* Pointer to the output array *)
  pOutByteArray: POINTER TO BYTE;
  (* Bit Pattern of Base 64 *)
  dwBitPattern: DWORD;
  (* Index of Base 64 to convert in ASCII *)
  ayBase64Index: ARRAY [0..3] OF BYTE;
  (* Converted Index in ASCII *)
  ayBase64Dec: ARRAY[0..3] OF BYTE;
  (* Enable padding One *)
  bPaddingOne: BOOL;
  (* Enable padding Two *)
  bPaddingTwo: BOOL;
  (* To calculate the Length at output *)
  dwInputAddress: DWORD;
END VAR
{flag off}


(*******************************************************************************
 * Programmed: 14.08.2009 - by Cristian Adamo <cristian.adamo@gmail.com>
...
IF (Enable) THEN
  ...
  FOR nSteps := 0 TO Size-1 BY 3	DO
    ...
    ayBase64Index[0] := DWORD_TO_BYTE(
                        SHR((dwBitPattern AND 2#111111000000000000000000), 18));
    ayBase64Index[1] := DWORD_TO_BYTE(
                        SHR((dwBitPattern AND 2#111111000000000000), 12));
    ayBase64Index[2] := DWORD_TO_BYTE(
                        SHR((dwBitPattern AND 2#111111000000), 6));
